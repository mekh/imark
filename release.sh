...
if [ "${1:-}" != "--force" ]; then
	...
	# In a folder of its own: it puts the renderer beside itself to serve it.
	mkdir -p /tmp/imark-release-editor-place
	swiftc -parse-as-library -I "$TEST_BIN" -I "$TEST_BIN/Modules" -F "$TEST_BIN" \
			-Xlinker -rpath -Xlinker "$TEST_BIN" \
			$(find Sources/Imark -name '*.swift' ! -name main.swift) \
			$(find Sources/ImarkRender -name '*.swift') \
			Support/test-editor-place.swift -o /tmp/imark-release-editor-place/run >/dev/null 2>&1 \
		&& /tmp/imark-release-editor-place/run >/dev/null || die "the editing place tests failed"
	swiftc -parse-as-library -I "$TEST_BIN" -I "$TEST_BIN/Modules" -F "$TEST_BIN" \
			-Xlinker -rpath -Xlinker "$TEST_BIN" \
			$(find Sources/Imark -name '*.swift' ! -name main.swift) \
			$(find Sources/ImarkRender -name '*.swift') \
			Support/test-recent.swift -o /tmp/imark-release-recent >/dev/null 2>&1 \
		&& /tmp/imark-release-recent >/dev/null || die "the Open Recent tests failed"
	...
	swift Support/test-type-scale.swift >/dev/null 2>&1 || die "the document text size tests failed"
	swift Support/test-diagram-viewer.swift >/dev/null 2>&1 || die "the diagram viewer tests failed"
	...
	# The same, for the same reason. Nothing in it reaches a real assistant.
	mkdir -p /tmp/imark-release-ask
	swiftc -parse-as-library -I "$TEST_BIN" -I "$TEST_BIN/Modules" -F "$TEST_BIN" \
			-Xlinker -rpath -Xlinker "$TEST_BIN" \
			$(find Sources/Imark -name '*.swift' ! -name main.swift) \
			$(find Sources/ImarkRender -name '*.swift') \
			Support/test-ask.swift -o /tmp/imark-release-ask/run >/dev/null 2>&1 \
		&& /tmp/imark-release-ask/run >/dev/null || die "the Ask tests failed"
